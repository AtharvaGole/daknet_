...
{	// 加载网络配置参数
    ...
    if(weights && weights[0] != 0){
        load_weights(net, weights);  // 如果存在权值 就加载权值
    }
    if(clear) (*net->seen) = 0;
    return net;
}
...
void update_network(network *netp)
{
    ...
    for(i = 0; i < net.n; ++i){
        layer l = net.layers[i];
        if(l.update){  // 如果该层可以update则..
            l.update(l, a);//每一层的参数更新
        }
    }
}
...
void backward_network(network *netp)
{
    ...
    for(i = net.n-1; i >= 0; --i){
        layer l = net.layers[i];
        if(l.stopbackward) break;  // 停止运算 退出
        if(i == 0){
            net = orig;
        }else{
            layer prev = net.layers[i-1];  // 前面一层
            // 找到前面一层的输出以及灵敏度图
            // 传递的是指针 指针的值改变 都变
