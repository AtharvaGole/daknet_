...
list *get_paths(char *filename)
{
    ...
    while((path=fgetl(file))){
        list_insert(lines, path); // 插入path到list当中 每一个path就是一个图片的存储地址
    }
    fclose(file);
    return lines;
}
...
{	// 分配n行内存空间
    ...
    for(i = 0; i < n; ++i){
        int index = rand()%m;
        random_paths[i] = paths[index]; // 随机选取一个路径并且赋值
        //if(i == 0) printf("%s\n", paths[index]);
    }
    // 释放锁
    pthread_mutex_unlock(&mutex);
...
box_label *read_boxes(char *filename, int *n)
{
    ...
    while(fscanf(file, "%d %f %f %f %f", &id, &x, &y, &w, &h) == 5){
        ...
        boxes[count].id = id;  //类别id  注意每一个图片当中的box数目是不一样的
        boxes[count].x = x;
        boxes[count].y = y;
        boxes[count].h = h;
        boxes[count].w = w;
...
*/
// -dx/w: 输出图与中间图差值/输出图宽度, -dy/h, nw/w: 中间图像/输出图像, nh/h
void correct_boxes(box_label *boxes, int n, float dx, float dy, float sx, float sy, int flip)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        // 坐标转换
        // sx = 中间图像/输出图像 即中间图像与输出图像比例
        // dx = -输出图与中间图差值/输出图宽度  计算的到 中间图像相对于输出图像的偏差值
        // left*sx相当于 中间图像的位置(原始图像与中间图像是同比例变化)
        boxes[i].left   = boxes[i].left  * sx - dx;  // 矫正偏差值
...
**       dy       此参数需要参考load_data_detection函数中的注释，dy是中间图相对最终图的起点位置的y坐标除以最终图的高度（并取负值）
...
*/
// -dx/w: 输出图与中间图差值/输出图宽度, -dy/h, nw/w, nh/h
void fill_truth_detection(char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy)
{
    ...
    // -dx/w: 输出图与中间图差值/输出图宽度 flip 是否进行了左右反转
    correct_boxes(boxes, count, dx, dy, sx, sy, flip);
    // 如果图片中含有的矩形框数多于num_boxes，则保持count = num_boxes，因为num_boxes是指定的每张图片最多参与训练的矩形框数，如果超过这个数，
    // 就在其中随机选择num_boxes个（box的顺序已经随机打乱了） 最多仅仅取出num_boxes个数据
...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
    ...
    char **random_paths = get_random_paths(paths, n, m);  // 随机选取n张图片
    int i;
    ...
    for(i = 0; i < n; ++i){
        ...
        // 创建一个 w h的空矩阵 并且初始化值为0.5
        image sized = make_image(w, h, orig.c);
        fill_image(sized, .5);
        // 缩放抖动大小：缩放抖动系数乘以原始图宽高即得像素单位意义上的缩放抖动
        // 计算缩放原始图像经过缩放抖动后的宽和长 允许抖动的上限和下限值
        float dw = jitter * orig.w;
        float dh = jitter * orig.h;
        // 中间图的长宽比（aspect ratio，此处其实是宽高比）：新的长宽比是一个随机数
        // 经过随机抖动后的 图像 宽度/长度 为了后面计算 以宽为主 还是以高为主
        ...
        // 二者之间的唯一的关系就是有一条边（宽或高, 代码里面的表现为哪一个长,哪一个起作用）的长度比例为scale
        float scale = rand_uniform(.25, 2);
        // nw,nh为中间图的宽高, new_ar为中间图的宽高比
        // 这个值根据宽高比计算得到的 宽高比例是不变的
        float nw, nh;
        ...
        // 这个是计算 输出图 中间图 之间的差值 输出图像像素点 相对于中间图像素点的位置
        ...
        // 原始图像 中间图 中间图与输出图的随机偏差 输出图
        place_image(orig, nw, nh, dx, dy, sized);
        // 随机对图像jitter（在hsv三个通道上添加扰动），实现数据增强
        // 针对hsv空间数据的抖动 hsv可以了解一下子
        random_distort_image(sized, hue, saturation, exposure);
    ...
    // 释放随机的路径
    free(random_paths);
    // attention 后面如果想用tf或者caffe实现yolo, 这个函数的实现是一个重要的地方,目前没有看见合适的框架
    // 后面看看ssd这个框架,可以对比一下子
    return d;
}
/**
...
void *load_thread(void *ptr)
{
    ...
    load_args a = *(struct load_args*)ptr;  // 数据的强制转换 为啥不直接传入 load_args* 类型 非要用*void
...
{   // 多线程读取数据, 这个可以在代码阅读差不多了,仔细研究c语言的这个部分,比如文本文件的处理时候,也可以使用多
    int i;
    ...
    *out = concat_datas(buffers, args.threads);
    ...
    for(i = 0; i < args.threads; ++i){
        buffers[i].shallow = 1;
        free_data(buffers[i]);  // 释放掉中间过程的数据 因为此时 数据已经给out了..
    }
...
{	// 获取next_batch数据
    int j;
    // 下面逐行将输入d中的数据深拷贝至X中
    for(j = 0; j < n; ++j){ // 拷贝n个记录
        int index = offset + j;  // 计算编译量
