...
void forward_rnn_layer(layer l, network net)
{
    ...
    // 遍历时间步长
    // http://blog.csdn.net/heyongluoyao8/article/details/48636251 这个博客讲的不错
    for (i = 0; i < l.steps; ++i) {
    	// 临时变量 存储第一层输入
        s.input = net.input;
        // 第一层全连接前向传播
...
void backward_rnn_layer(layer l, network net)
{
    ...
    for (i = l.steps-1; i >= 0; --i) {
        ...
        // l.state = l.state + s_(t-1)*W = x_t * U + s_(t-1)*W
        axpy_cpu(l.outputs * l.batch, 1, self_layer.output, 1, l.state, 1);
        // o_t = s_t * V = (x_t * U + s_(t-1)*W) * V rnn的输出结果
        s.input = l.state;
        // 注意 w_t的值会由w_(t+1)更新一部分 确信这里有debug
        s.delta = output_layer.delta;
        // 按照时间展开的后向传播算法 计算得到V_t的delta
