...
struct layer{
    ...
    ACTIVATION activation;   // 层激活函数
    COST_TYPE cost_type;     // 代价函数类型
    // 前向 后向 更新函数 全都是函数的指针哦
    ...
    int n;        // 对于卷积层，该参数表示卷积核个数，等于out_c，其值由网络配置文件指定；对于region_layerc层，该参数等于配置文件中的num值
                  ...
                  // 可以在darknet/cfg文件夹下执行命令：grep num *.cfg便可以搜索出所有设置了num参数的网络，这里面包括yolo.cfg等，其值有设定为3,5,2的。
...
typedef struct network{
    ...
    float *output;
    learning_rate_policy policy;  // 学习率衰减方式

    float learning_rate;          // 初始化的学习率
    float momentum;               // 惯性
    float decay;                  //
    float gamma;                  //
    float scale;                  //
